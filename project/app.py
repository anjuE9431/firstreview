...
def admin_fish():
    ...
    if request.method == "POST":
        ...
        harbour_id = request.form["harbour_id"]
        cur.execute(
            "INSERT INTO fish(name,price,stock,harbour_id) VALUES(%s,%s,%s,%s)",
            (name, price, stock, harbour_id),
        )
        con.commit()
        flash("Fish added.", "success")
    cur.execute(
        """
        SELECT f.*, h.name as harbour_name FROM fish f
        JOIN harbours h ON h.id=f.harbour_id ORDER BY f.id DESC
        """
    )
    fish = cur.fetchall()
    cur.execute("SELECT id,name FROM harbours")
...
def harbour_orders():
    ...
    cur.execute("SELECT id,name FROM users WHERE role='delivery'")
